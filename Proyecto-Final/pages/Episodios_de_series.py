...
def load_main_data():
    try:
        ...
        df_episodes_raw = pd.read_csv('data/title.episode.tsv', sep='\t', encoding='utf-8', na_values='\\N')

        # Limpieza y preparación para df_combined
        ...
        # Limpieza y preparación para df_episodes_raw
        ...
        st.error(f"Error al cargar archivos CSV/TSV para el gráfico de barras: {e}. Asegúrate de que 'data/imdb_dataset.csv' y 'data/title.episode.tsv' existan.")
        return pd.DataFrame()
    except Exception as e:
        st.error(f"Error inesperado al cargar datos para el gráfico de barras: {e}")
        return pd.DataFrame()

@st.cache_data
def load_episode_ratings_data():
    CSV_FILE_PATH = 'data/imdb_episodios.csv'
    try:
        if not os.path.exists(CSV_FILE_PATH):
            st.error(f"Error: El archivo '{CSV_FILE_PATH}' no se encontró para el gráfico de calificaciones de episodios.")
            st.warning("Por favor, asegúrate de que tu script de pre-procesamiento lo haya guardado correctamente.")
            return pd.DataFrame()
        
        df_ep_ratings = pd.read_csv(CSV_FILE_PATH)
        return df_ep_ratings
    except Exception as e:
        st.error(f"Ocurrió un error al cargar el archivo CSV '{CSV_FILE_PATH}' para el gráfico de calificaciones: {e}")
        return pd.DataFrame()
